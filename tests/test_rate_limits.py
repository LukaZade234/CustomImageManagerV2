...
class TestEndpoints:
    def test_a_burst_gets_429_with_retry_after(self, client, clean_db, identity_id, monkeypatch):
        monkeypatch.setitem(upload_imgchest.RATE_LIMITS, "hide", [(3, 60)])
        for _ in range(3):
    ...
    def test_the_limit_does_not_leak_between_actions(
        ...
        monkeypatch.setitem(upload_imgchest.RATE_LIMITS, "hide", [(1, 60)])
        monkeypatch.setitem(upload_imgchest.RATE_LIMITS, "report", [(5, 60)])
    ...
    def test_reports_are_limited_more_tightly_than_hides(self):
        ...
        report_hourly = by_window(upload_imgchest.RATE_LIMITS["report"])[3600]
        hide_hourly = by_window(upload_imgchest.RATE_LIMITS["hide"])[3600]
...
class TestConfiguration:
    def test_limits_can_be_overridden_by_environment(self, monkeypatch):
        ...
        assert upload_imgchest._limits_from_env("upload", [(1, 1)]) == [(5, 60), (50, 3600)]
    ...
    def test_a_malformed_override_falls_back_to_the_default(self, monkeypatch):
        ...
        assert upload_imgchest._limits_from_env("upload", [(7, 60)]) == [(7, 60)]
    ...
    def test_every_action_used_by_a_route_has_limits_defined(self):
        ...
        for action, windows in upload_imgchest.RATE_LIMITS.items():
